...
def auto_expand_paths_text():
    ...
    lines = min(max(lines, 10), 20)  # от 10 до 20 строк
...
# Обработчик горячих клавиш для работы в любой раскладке
def keypress_handler(event):
    ctrl_pressed = (event.state & 0x4) != 0
    if ctrl_pressed:
        # keycode для C, V, X, Z стандартные
        if event.keycode == 67:  # C
            ...
            return "break"
        elif event.keycode == 90:  # Z (undo, если нужно)
            # event.widget.event_generate('<<Undo>>') - можно реализовать
            return "break"

def bind_standard_shortcuts(widget):
...
paths_text = scrolledtext.ScrolledText(right_frame, width=80, height=10)
...
# Привязываем обработчик горячих клавиш ко всем основным полям
bind_standard_shortcuts(paths_text)
bind_standard_shortcuts(output_entry)
bind_standard_shortcuts(tree_text)
